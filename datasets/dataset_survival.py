...
class SurvivalDatasetFactory:
    ...
    def _setup_mcat(self):
        ...
        for col in self.signatures.columns:
            ...
            self.genomic_names.append(omic)
        self.genomic_sizes = [len(omic) for omic in self.genomic_names]
    ...
    def _setup_survpath(self):
        ...
        self.transomic_names = []
        for col in self.signatures.columns:
            omic = self.signatures[col].dropna().unique()
            omic = sorted(_series_intersection(omic, self.all_modalities["rna"].columns))
            self.transomic_names.append(omic)
        self.transomic_sizes = [len(omic) for omic in self.transomic_names]
    ...
    def return_splits(self, args, csv_path, fold):
        ...
        args.transomic_sizes = args.dataset_factory.transomic_sizes
        args.genomic_sizes = args.dataset_factory.genomic_sizes
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str='train', fold = None, scaler=None, valid_cols=None):
        ...
        split_dataset = SurvivalDataset(
            ...
            transomic_names = self.transomic_names,
            genomic_names = self.genomic_names,
            sample=sample
            )
...
class SurvivalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.num_patches = num_patches
        self.transomic_names = transomic_names
        self.genomic_names = genomic_names
        self.num_pathways = len(transomic_names)
        self.sample = sample
        self.table_dir = os.path.join('clinic_embedding/clip/', self.study_name)
        # self.adj = self.to_sparse(self.adj)
         
        # for weighted sampling
        self.slide_cls_id_prep()
        
        # path = os.path.join('protein_files', 'sim', '{}.csv'.format(self.study_name))
        
        self.sim_protein = pd.read_csv(path)
    ...
    def __getitem__(self, idx):
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        ...
        elif self.modality in ["coattn", "coattn_motcat", "hump"]:
            
            patch_features, patient_name = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            table_file = os.path.join(self.table_dir, '{}.pt'.format(slide_ids[0][:12]))
            table_features = torch.load(table_file)
            protein = self.get_protein(slide_ids[0])
            ...
            return (patch_features, protein, genomic_list, transomic_list, table_features, label, event_time, c, clinical_data, patient_name)
    ...
    def build_gene_graph(self, genomics_features, transomic_features):
        edges = []
    
        genomics_gene_to_nodes = {}
        for i in range(6):
            for gene in genomics_features[i]:
                if gene not in genomics_gene_to_nodes:
                    genomics_gene_to_nodes[gene] = set()
                genomics_gene_to_nodes[gene].add(i)
    
        for j in range(331):
            for gene in transomic_features[j]:
                if gene in genomics_gene_to_nodes:
                    for i in genomics_gene_to_nodes[gene]:
                        edges.append((i, j + 6))
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        if True:
            try:
                graph_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                # print(graph_path)
                graph = torch.load(graph_path, weights_only=False)
                patch_features.append(graph)
            except FileNotFoundError:
                print('FileNotFound: ', graph_path)
        #try:
        if True:
            self.num_patches = 40000
            ...
            if patch_features.size(0)>self.num_patches:
                ...
                # graph_representations[0].x = patch_features[indices]

        return patch_features, slide_id
        
    def get_protein(self, patient_id):
        path = os.path.join('protein_files', self.study_name,'{}.pt'.format(patient_id[:12].rstrip('.svs')))
        # print(path)
        
        if os.path.exists(path):
            protein_tensor = torch.load(path)
            return protein_tensor
        else:
            return None
    
    def get_sim_protein(self, patient_id):
        # print(patient_id)
        patient_id = patient_id[:12]
        if patient_id not in self.sim_protein['patient_id'].values:
             # print('404')
             return None
    
        patient_row = self.sim_protein[self.sim_protein['patient_id'] == patient_id]
    
        protein_values = patient_row.iloc[0, 1:].tolist()
    
        try:
             tensor_values = torch.tensor(protein_values, dtype=torch.float32).unsqueeze(0)
             # print(tensor_values)
