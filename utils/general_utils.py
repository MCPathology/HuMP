...
def _collate_MCAT(batch):
    r"""
    Collate function for HuMP (pathways version) model
    ...
    omic1 = torch.cat([item[2][0] for item in batch], dim = 0).type(torch.FloatTensor)
    omic2 = torch.cat([item[2][1] for item in batch], dim = 0).type(torch.FloatTensor)
    omic3 = torch.cat([item[2][2] for item in batch], dim = 0).type(torch.FloatTensor)
    omic4 = torch.cat([item[2][3] for item in batch], dim = 0).type(torch.FloatTensor)
    omic5 = torch.cat([item[2][4] for item in batch], dim = 0).type(torch.FloatTensor)
    omic6 = torch.cat([item[2][5] for item in batch], dim = 0).type(torch.FloatTensor)
    genomic_list = [omic1, omic2, omic3, omic4, omic5, omic6]

    transomic_list = []
    for item in batch:
        transomic_list.append(item[3])
    adj = batch[0][4]
    label = torch.LongTensor([item[5].long() for item in batch])
    event_time = torch.FloatTensor([item[6] for item in batch])
    c = torch.FloatTensor([item[7] for item in batch])

    clinical_data_list = []
    for item in batch:
        clinical_data_list.append(item[8])
    patient = batch[0][9]


    # mask = torch.stack([item[11] for item in batch], dim=0)

    return [img, protein, genomic_list, transomic_list, adj, label, event_time, c, clinical_data_list, patient]
...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    elif args.modality in ["coattn", "coattn_motcat", "hump"]:
