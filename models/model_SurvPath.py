...
class SurvPath(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # 计算权重
        # 尝试将embedding结果作为x, out相当于再过一次线性层 
        #
        '''path_f = h_omic_bag.clone()
        wsi_f = wsi_embed.clone()'''
        # shape change
        #print(path_f.shape)
        # print(wsi_f.shape)
        # shape:[1,4096,256]->[1,8]
        


        '''if kwargs["model"] == "train":
            path_tcp = self.ConfidNet_p_2(self.ConfidNet_p_1(path_f).view(1,-1))
            wsi_tcp = self.ConfidNet_h_2(self.ConfidNet_h_1(wsi_f).view(1,-1))
        
            path_holo = torch.log(wsi_tcp)/(torch.log(path_tcp*wsi_tcp)+1e-8)
            wsi_holo = torch.log(path_tcp)/(torch.log(path_tcp*wsi_tcp)+1e-8)
            cb_path = path_tcp.detach() + path_holo.detach()
            cb_wsi = wsi_tcp.detach() + wsi_holo.detach()
            w_all = torch.stack((cb_path, cb_wsi),1)
            softmax = nn.Softmax(1)
            w_all = softmax(w_all)
            w_path = w_all[:,0]
            w_wsi = w_all[:,1]
        elif kwargs["model"]=="eval":
            path_out = self.pathclf(self.pathcat(path_f).view(1,-1))
            wsi_out = self.wsiclf(self.wsicat(wsi_f).view(1,-1))
            path_tcp = self.ConfidNet_p_2(self.ConfidNet_p_1(path_f).view(1,-1))
            wsi_tcp = self.ConfidNet_h_2(self.ConfidNet_h_1(wsi_f).view(1,-1))
            path_holo = torch.log(wsi_tcp)/(torch.log(path_tcp*wsi_tcp)+1e-8)
            wsi_holo = torch.log(path_tcp)/(torch.log(path_tcp*wsi_tcp)+1e-8)
            cb_path = path_tcp + path_holo
            cb_wsi = wsi_tcp + wsi_holo
            path_pred = torch.nn.functional.softmax(path_out, dim=1)
            wsi_pred = torch.nn.functional.softmax(wsi_out, dim=1)
            path_du = torch.mean(torch.abs(path_pred - 1 / path_pred.shape[1]), dim=1, keepdim=True)
            wsi_du = torch.mean(torch.abs(wsi_pred - 1 / wsi_pred.shape[1]), dim=1, keepdim=True)
            condition = path_du > wsi_du
            rc_t = torch.where(condition,torch.ones_like(path_du),path_du/wsi_du)
            rc_i = torch.where(condition,wsi_du/path_du,torch.ones_like(wsi_du))
            ccb_path = cb_path * rc_t
            ccb_wsi = cb_wsi * rc_i
            w_all = torch.stack((ccb_path,ccb_wsi),1)
            softmax = nn.Softmax(1)
            w_all = softmax(w_all)
            w_path = w_all[:,0]
            w_wsi = w_all[:,1]'''
