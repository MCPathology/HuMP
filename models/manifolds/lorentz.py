...
import models.manifolds.lmath as math
from models.manifolds.utils import acosh
...
class Lorentz(LorentzOri):
    ...
    def proj(self, x):
        ...
        vals[:, 0:1] = torch.sqrt(torch.clamp(max=10., min=self.k-y_sqnorm))
        return vals + mask * x
    ...
    def activation(self, x: torch.Tensor, activation, add_time: bool=True) -> torch.Tensor:
